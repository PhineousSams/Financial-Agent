defmodule FinincialAgentWeb.Helps.PaginationComponent1 do
  @moduledoc false
    ...
    <div>
      ...
      <div class="row">
        <div class="col-sm-12 col-md-7">
          <ul id={assigns[:id] || "pagination"} class="pagination" style="margin: auto;">
            <%= if @total_pages > 1 do %>
              <li class="page-item"><%= prev_link(@params, @page_number) %></li>

              <%= for num <- start_page(@page_number)..end_page(@page_number, @total_pages) do %>
                <li class={"page-item #{if @page_number == num, do: "active"}"}>
                  <%= live_patch(num,
                    to: "?#{querystring(@params, page: num)}",
                    class: "page-link #{if @page_number == num, do: "active", else: ""}"
                  ) %>
                </li>
              <% end %>
