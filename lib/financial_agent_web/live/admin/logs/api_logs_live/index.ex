defmodule FinincialAgentWeb.ApiLogsLive.Index do
  ...
  def handle_params(params, _url, socket) do
    if connected?(socket), do: send(self(), {:api_logs, params})
  ...
  defp apply_action(socket, :index, _params) do
    socket
    |> assign(:page_title, "Bill API Logs")
    |> assign(:log, nil)
  end
  ...
  defp handle_event_switch(target, params, socket) do
    case target do
      ...
      "reload" -> {:noreply, push_navigate(socket, to: ~p"/mobileBanking/bills/api-logs")}
  ...
  defp get_logs(%{assigns: _assigns} = socket, params) do
    data = Logs.get_api_logs(Control.create_table_params(socket, params))
