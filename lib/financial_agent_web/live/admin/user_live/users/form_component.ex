defmodule FinancialAgentWeb.Admin.UserLive.FormComponent do
  @moduledoc false
  ...
  defp handle_user(%{assigns: assigns}, %{user: user}, :new) do
    ...
    |> Ecto.Multi.run(:logs, fn _repo, %{user: user} ->
      activity = "Updated an admin user named: #{user.first_name <> " " <> user.last_name} with id: #{user.id}"
