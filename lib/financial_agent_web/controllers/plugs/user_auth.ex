defmodule FinincialToolWeb.Plugs.UserAuth do
  import Plug.Conn
  ...
  @max_age 60 * 60 * 24 * 60
  @remember_me_cookie "_FinincialTool_web_user_remember_me"
  ...
  def log_in_user(conn, user, params \\ %{}) do
    ...
    |> redirect(to: redirect_to || signed_in_path(user.user_type))
  end
  ...
  def log_out_user(conn) do
    ...
    if live_socket_id = get_session(conn, :live_socket_id) do
      FinincialToolWeb.Endpoint.broadcast(live_socket_id, "disconnect", %{})
    end
  ...
  def fetch_current_user(conn, _opts) do
    ...
    type = if(conn.request_path == "/Admin", do: "ADMIN", else: "CLIENT")
    ...
    case user_token && Accounts.get_user_by_session_token(user_token, type) do
      ...
      user ->
        assign(conn, :current_user, user) |> assign(:user, user)
    end
  end
  ...
  def redirect_if_user_is_authenticated(conn, _opts) do
    if conn.assigns[:user] do
      conn
      |> delete_session(:last_accessed_path)
      |> redirect(to: signed_in_path(conn.assigns.current_user.user_type))
      |> halt()
    else
      conn
    end
