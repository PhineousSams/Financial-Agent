defmodule FinincialTool.Workers.Util.Utils do
  ...
  # FinincialTool.Workers.Util.Utils.date_format("28/03/2024")
  ...
  def traverse_errors2(errors) do
    errors = for {_key, {msg, _opts}} <- errors, do: "#{msg}"
    List.first(errors)
  end
  ...
  def country_code_checker(number) do
    if is_binary(number) do
      cleaned_phone = String.replace(number, ~r/\D/, "")

      case cleaned_phone do
        <<"260", rest::binary>> ->
          if String.length(rest) == 9 do
            "260" <> rest
          else
            nil
          end
...
# FinincialTool.Workers.Util.Utils.gen_ref()
